...
print(f'Target Score (0 to 20, assume no penalt): = {np.round(target_est_score, 3)}')
```
...
Your robot has to perform a [qualified navigation run](../Week08-09/M4_marking.md#evaluation) fully autonomously in order to get marks for this sub-task. It can either use the full arena map you have generated with your SLAM and CV compoents (Level 2 of M4) or dynamically replan its path when encountering an obstacle (Level 3 of M4). 
...
16. The robot must start at the origina (0,0,0) for the navigation task. You can't teleoperate or manually place the robot next to the first target when starting the navigation.
...
3. Use any team member's account to log in Moodle and nagviate to the M5 submission box, so that you are ready to download your submitted code when the demonstrator arrives
...
#### Step 4:
**Right after marking**
1. Put all your ```slam.txt``` and ```targets.txt``` map files (after renaming them following the format in Setp 3) in a folder named M5_maps_{team_number} (e.g. M5_maps_404), then zip and submit M5_maps_{team_number}.zip on Moodle. 
