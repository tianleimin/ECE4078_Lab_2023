...
class Robot:
    ...
    def covariance_drive(self, drive_meas):
        ...
        if ang_vel == 0:
            self.state[0] += np.cos(self.state[2]) * lin_vel * dt
            self.state[1] += np.sin(self.state[2]) * lin_vel * dt
            Jac2[0, 0] = self.state[0]
            Jac2[0, 1] = 0
            Jac2[1, 0] = self.state[1]
            Jac2[1, 1] = 0
            Jac2[2, 1] = 0
            Jac2[2, 0] = 0
            ...
            #th = self.state[2]
            #self.state[0] += lin_vel / ang_vel * (np.sin(th+dt*ang_vel) - np.sin(th))
            #self.state[1] += -lin_vel / ang_vel * (np.cos(th+dt*ang_vel) - np.cos(th))
            #self.state[2] += dt*angular_velocity

            Jac2[0, 0] = 1/ang_vel*(-np.sin(th)+np.sin(th+dt*ang_vel))
            Jac2[0, 1] = (lin_vel/ang_vel**2)*(-np.sin(th)+np.sin(th+dt*ang_vel))*dt
            Jac2[1, 0] = 1/ang_vel*(np.cos(th)-np.cos(th+dt*ang_vel))
            Jac2[1, 1] = (lin_vel/ang_vel**2)*(np.cos(th)-np.cos(th+dt*ang_vel))*dt
            Jac2[2, 0] = 0
            Jac2[2, 1] = dt
